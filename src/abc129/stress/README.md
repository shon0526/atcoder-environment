...
1. 愚直解を作成: `src/abc129/src/bin/<problem>_naive.rs`
...
cargo build --release -p abc129

# 問題と試行回数を指定して実行
uv run python src/abc129/stress/stress.py <problem> <試行回数>
```
...
```bash
target/release/abc129-<problem> < src/abc129/stress/ng_<problem>.txt
```

## メモ

- 提出バイナリ名は cargo-compete 設定に合わせて `abc129-<problem>`、
