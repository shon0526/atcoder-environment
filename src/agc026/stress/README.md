...
1. 愚直解を作成: `src/agc026/src/bin/<problem>_naive.rs`
...
cargo build --release -p agc026

# 問題と試行回数を指定して実行
uv run python src/agc026/stress/stress.py <problem> <試行回数>
```
...
```bash
target/release/agc026-<problem> < src/agc026/stress/ng_<problem>.txt
```

## メモ

- 提出バイナリ名は cargo-compete 設定に合わせて `agc026-<problem>`、
