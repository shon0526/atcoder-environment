...
1. 愚直解を作成: `src/abc464/src/bin/<problem>_naive.rs`
...
cargo build --release -p abc464

# 問題と試行回数を指定して実行
uv run python src/abc464/stress/stress.py <problem> <試行回数>
```
...
```bash
target/release/abc464-<problem> < src/abc464/stress/ng_<problem>.txt
```

## メモ

- 提出バイナリ名は cargo-compete 設定に合わせて `abc464-<problem>`、
