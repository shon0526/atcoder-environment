...
1. 愚直解を作成: `src/__CONTEST__/src/bin/<problem>_naive.rs`
...
cargo build --release -p __CONTEST__

# 問題と試行回数を指定して実行
uv run python src/__CONTEST__/stress/stress.py <problem> <試行回数>
```
...
```bash
target/release/__CONTEST__-<problem> < src/__CONTEST__/stress/ng_<problem>.txt
```

## メモ

- 提出バイナリ名は cargo-compete 設定に合わせて `__CONTEST__-<problem>`、
